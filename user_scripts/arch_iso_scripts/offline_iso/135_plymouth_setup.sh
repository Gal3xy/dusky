...
# Updated to target the nested assets directory relative to script execution
readonly ASSETS_DIR="./assets/plymouth"
readonly MKINITCPIO_CONF="/etc/mkinitcpio.conf.d/10-arch-btrfs-luks.conf"
readonly LIMINE_SCRIPT="./155_limine_setup.sh"
...
if ! pacman -Q plymouth >/dev/null 2>&1; then
    # Explicit user directive: Install without triggering a full system sync/upgrade
    pacman -S --needed --noconfirm plymouth || fatal "Failed to install plymouth. Check mirrorlist or network."
fi
...
if [[ ! -d "$ASSETS_DIR" ]]; then
    fatal "Assets directory '$ASSETS_DIR' not found. Please place your PNG files there."
fi
...
if [[ -f "$MKINITCPIO_CONF" ]]; then
    # Strictly match active (uncommented) HOOKS array and preserve symlinked drop-ins
...
if [[ -f "$LIMINE_SCRIPT" ]]; then
    if ! grep -q '"splash"' "$LIMINE_SCRIPT"; then
        # Use a flexible regex address to find the target array assignment
        ...
        # Verify post-execution that the parameters were successfully injected
        if grep -q '"splash"' "$LIMINE_SCRIPT"; then
